...
class WindowManager(ScreenManager):
	pass
class  Closebtn(MDIconButton,ButtonBehavior):
	pass
...
class Picture_Screen(Screen):
	def __init__(self,**kwargs):
		super(Picture_Screen,self).__init__(**kwargs)
	def on_enter(self):
	...
	def Android_back_click(self,window,key,*largs):
		...
		#Clock.schedule_once(self.class_method,1/60)
					
	def class_method(self,imgsource):
		self.imgsource=imgsource
		
		App.get_running_app().root.current='picture_screen'
...
class Imagelist(MDSmartTile,ButtonBehavior):
	def __init__(self,**kwargs):
	...
	def on_release(self):
		...
		try:
			...
			img_id=App.get_running_app().root.get_screen('picture_screen').ids.image
			img_id.source=(' ')
			img_id.source=str(img_path)    
		except Exception as er:
			Logger.exception('Picture: unable to load <%s>' % img_path)
		ps=Picture_Screen()
		ps.class_method(img_path)
	def on_pause(self):
		return True
		
		
		#Clock.schedule_once(obj.class_method(img_path),1/60)
...
#    	self.ads.show_banner()   
    ...
    def show_selected_img(self,path):
       ...
       # update image
        self.manager.get_screen('upload_screen').ids.upload_btn.source=(path)
    ...
    #here append url in main.txt
                ...
                self.ids.upload_btn.source=" "
                self.ids.upload_btn.source='upload img.png'
...
#    	self.ads.request_banner()
#    	self.ads.show_banner()
    	Window.bind(on_keyboard=self.android_event)  
...
#---------MDFileManager--------
    def on_enter(self):
        ...
        self.animate_btn_up()
        self.anim(self.manager.get_screen('account_screen').ids.first_layout)
        self.anim1(self.manager.get_screen('account_screen').ids.second_layout)
        self.anim2(self.manager.get_screen('account_screen').ids.third_layout)
        self.anim3(self.manager.get_screen('account_screen').ids.forth_layout)
        Window.bind(on_keyboard=self.android_event)
...
#--------------------MDFileManager--------------
    ...
    def select_path(self, path):      
        ...
        if file_size >=1.0:           
           ...
           #os.remove('/storage/emulated/0/DCIM/pythonthumb.png')
            img_path=image.save('./'+str(file_list[0])+'.png')
            print(img_path)
    ...
    def events(self,window,keyboard,*largs):
        '''Called when buttons are pressed on the mobile device.'''
        if keyboard in (1001, 27):
        	if self.manager_open:
...
#----------animate button-----------       
    def animate_btn_up(self,*args): 
        action_btn=self.manager.get_screen('account_screen').ids.action_btn
...
#---------toolbar backspace----------- 
    def screen_back(self):       
        self.btn.cancel(self.manager.get_screen('account_screen').ids.action_btn)
        self.manager.current='main_screen'                       
#---------profile pic change---------            
...
#-----------profile save----------
    ...
    def  android_event(self,window,key,*largs):
    	if key==27:
    		print('pressed')
    		self.manager.current='main_screen'
    		return True
...
#            self.manager.get_screen('account_screen').ids.spinnerbar.active=False
    ...
    def profile_save(self):
        ...
        if account_name=='guest':
            ...
            update_name=self.manager.get_screen('account_screen').ids.name_text.text                    
            update_username=self.manager.get_screen('account_screen').ids.username_text.text
            update_password=self.manager.get_screen('account_screen').ids.password_text.text
        ...
        try:
            with open('main.txt','r') as img_file:
                ...
                for i in reversed(image_list):               
                	web_img=Imagelist(source=i,size_hint_y= None,height="240dp") #size_hint=(None,None),height='170dp',width='170dp')
...
#            print(er)
    ...
    def show_theme_picker(self):
        toast('please Wait...')
        self.theme_dialog = MDColorPicker()
...
#        anim.start(widget)
    ...
    def  login_func(self):
        try:
            ...
            self.dialog = MDDialog(title='Error', radius= [20],text=str(e),buttons=[MDRaisedButton(text='Try Again',on_release=lambda x:self.login_func())],auto_dismiss=False )
            self.dialog.open()               
